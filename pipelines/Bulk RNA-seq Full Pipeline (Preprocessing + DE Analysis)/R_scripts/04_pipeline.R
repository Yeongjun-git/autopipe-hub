...
# [MODIFIED] Hallmark_path 인자 추가 — GO_DEG_analysis, GSEA_analysis에 전달
analysis_data = function(path, Data, method, FC, pval, Comparison_name, Sample, Keytype, norm_method="edgeR", specific_geneset = NULL, specific_genes = NULL, Hallmark_path = NULL){
    ...
    GO_DEG_analysis(path = path, data = data, Organism = Sample, Comparison_name = Comparison_name, Keytype = Keytype, Custom_geneset_gmt = specific_geneset, Hallmark_path = Hallmark_path)

    GSEA_analysis(path = path, data = data, Organism = Sample, Comparison_name = Comparison_name, Keytype = Keytype, Hallmark_path = Hallmark_path)
