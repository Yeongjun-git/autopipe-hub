...
# Parse comb_set: expects JSON-like format e.g. '[["A","B"],["C","D"]]'
comb_set_raw <- params$comb_set
comb_set_raw <- gsub("'", '"', comb_set_raw)
comb_set <- tryCatch(
  jsonlite::fromJSON(comb_set_raw, simplifyVector = FALSE),
  error = function(e) {
    # Fallback: parse simple format "A,B;C,D"
    pairs <- strsplit(comb_set_raw, ";")[[1]]
    lapply(pairs, function(p) strsplit(trimws(p), ",")[[1]])
  }
)

# Parse specific_genes
specific_genes_raw <- params$specific_genes
if (!is.null(specific_genes_raw) && specific_genes_raw != "" && specific_genes_raw != "[]") {
  specific_genes_raw <- gsub("[\\[\\]'\"]", "", specific_genes_raw)
  specific_genes <- trimws(strsplit(specific_genes_raw, ",")[[1]])
  specific_genes <- specific_genes[specific_genes != ""]
} else {
  specific_genes <- NULL
}
...
if (is.null(specific_geneset) || specific_geneset == "") {
  specific_geneset <- NULL
} else if (!file.exists(specific_geneset)) {
  message("Warning: specific_geneset path does not exist: ", specific_geneset)
  specific_geneset <- NULL
}

# Hallmark GMT path (override internal default)
hallmark_gmt <- params$hallmark_gmt
if (!is.null(hallmark_gmt) && hallmark_gmt != "" && file.exists(hallmark_gmt)) {
  # Override the hardcoded Hallmark path in the enrichment functions
